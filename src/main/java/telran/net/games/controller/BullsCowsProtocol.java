...
public class BullsCowsProtocol implements Protocol {
	...
	private Response getNotStartedGamesWithNoGamer(String requestData) {
		List<Long> result = bcService.getNotStartedGamesWithNoGamer(requestData);
		String responseString = resultsToJSON(result);
		return getResponseOk(responseString );
	}

	private Response getNotStartedGamesWithGamer(String requestData) {
		List<Long> result = bcService.getNotStartedGamesWithGamer(requestData);
		String responseString = resultsToJSON(result);
		return getResponseOk(responseString );
	}
	...
	Response registerGamer(String requestData) {
		UsernameBirthdateDto dto = new UsernameBirthdateDto( new JSONObject(requestData) );
		bcService.registerGamer(dto.username(), dto.birthdate());
	...
	Response getNotStartedGames(String requestData) {
		List<Long> results = bcService.getNotStartedGames();
	...
	Response gameOver(String requestData) {
		boolean result = bcService.gameOver(Long.parseLong(requestData));
		String responseString = Boolean.toString(result);
		return getResponseOk(responseString );
	}
	Response getGameGamers(String requestData) {
		List<String> result = bcService.getGameGamers(Long.parseLong(requestData));
		String responseString = resultsToJSON(result);
		return getResponseOk(responseString );
	}
